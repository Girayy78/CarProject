    ...
    public void initialize() {
        try (Connection conn = DataBase.connect()) {
            PreparedStatement stmt = conn.prepareStatement(
                    "SELECT car_brand, car_model, plate FROM users WHERE name = ?"
            );
    ...
    private void registerCar(ActionEvent event) {
        ...
        try (Connection conn = DataBase.connect()) {
            ...
            PreparedStatement updateStmt = conn.prepareStatement(
                    "UPDATE users SET car_brand = ?, car_model = ?, plate = ? WHERE name = ?"
            );
            updateStmt.setString(1, brand);
            updateStmt.setString(2, model);
            updateStmt.setString(3, plate);
            updateStmt.setString(4, LoggedUser.getUsername());
